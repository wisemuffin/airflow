        ...
        while len(tweets) < self.max_tweets:
            ...
            try:
                new_tweets = api.search(
                    q=query + " -filter:retweets", count=count, max_id=str(last_id - 1), tweet_mode='extended')
                if not new_tweets:
                    break
    ...
    def get_trends(self, api):
        ...
        for trend in trend_list:
            if trend['tweet_volume']:
                trends.append(trend)
        trends = sorted(trends, key=lambda trend: (
            trend['tweet_volume']), reverse=True)
