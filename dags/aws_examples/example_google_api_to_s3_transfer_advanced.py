...
YOUTUBE_CHANNEL_ID = getenv(
    "YOUTUBE_CHANNEL_ID", "UCSXwxpWZQ7XZ1WL3wqevChA")  # "Apache Airflow"
...
def _check_and_transform_video_ids(xcom_key, task_ids, task_instance, **kwargs):
    video_ids_response = task_instance.xcom_pull(
        task_ids=task_ids, key=xcom_key)
    video_ids = [item['id']['videoId'] for item in video_ids_response['items']]

    if video_ids:
        task_instance.xcom_push(key='video_ids', value={
                                'id': ','.join(video_ids)})
...
with DAG(
    ...
    task_check_and_transform_video_ids = BranchPythonOperator(
        ...
        op_args=[task_video_ids_to_s3.google_api_response_via_xcom,
    ...
    task_video_ids_to_s3 >> task_check_and_transform_video_ids >> [
        task_video_data_to_s3, task_no_video_ids]
